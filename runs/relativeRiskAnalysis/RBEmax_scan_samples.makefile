...
# This makefile is used for producing 1D scans of the RBE_min parameter
...
RBEMAX_CION_BLADDER_LOW = 1
RBEMAX_CION_BLADDER_HIGH = 9
...
RBEMAX_PROTON_BLADDER_LOW = 1.05
RBEMAX_PROTON_BLADDER_HIGH = 1.6
...
CION_BLADDER_FILELIST = $(foreach N,$(FILEIDS),RBEmax_scan_cion_bladder_samples_$(N).mat)
CION_RECTUM_FILELIST = $(foreach N,$(FILEIDS),RBEmax_scan_cion_rectum_samples_$(N).mat)
PROTON_BLADDER_FILELIST = $(foreach N,$(FILEIDS),RBEmax_scan_proton_bladder_samples_$(N).mat)
PROTON_RECTUM_FILELIST = $(foreach N,$(FILEIDS),RBEmax_scan_proton_rectum_samples_$(N).mat)
FILELIST = $(CION_BLADDER_FILELIST) $(CION_RECTUM_FILELIST) $(PROTON_BLADDER_FILELIST) $(PROTON_RECTUM_FILELIST)

OUTPUT_FILES = RBEmax_scan_cion_bladder_sample_data.mat \
               RBEmax_scan_cion_rectum_sample_data.mat \
               RBEmax_scan_proton_bladder_sample_data.mat \
               RBEmax_scan_proton_rectum_sample_data.mat
...
cleanall: clean
	rm -rf make_RBEmax_scan_samples merge_RBEmax_scan_samples $(OUTPUT_FILES)


define merge_RBEmax_scan_samples
#!/bin/sh
outfile="$$1"
shift
...
EOF
endef
export merge_RBEmax_scan_samples

merge_RBEmax_scan_samples: RBEmax_scan_samples.makefile
	echo "$$merge_RBEmax_scan_samples" > $@
	chmod +x $@


define make_RBEmax_scan_samples
#!/bin/sh
exec $(OCTAVE) <<EOF
Nsamples = $$2;
...
RBEmax_distrib = struct('type', 'box', 'params', {{$$17, $$18}});
...
EOF
endef
export make_RBEmax_scan_samples

make_RBEmax_scan_samples: RBEmax_scan_samples.makefile
	echo "$$make_RBEmax_scan_samples" > $@
	chmod +x $@


$(CION_BLADDER_FILELIST): make_RBEmax_scan_samples
	./make_RBEmax_scan_samples $@ $(N_SAMPLES) 'data/VMATdvh/vmat%d.mat' 25 1 \
		...
		$(RBEMAX_CION_BLADDER_LOW) $(RBEMAX_CION_BLADDER_HIGH) \
		> log_$@.txt 2>&1


RBEmax_scan_cion_bladder_sample_data.mat: merge_RBEmax_scan_samples $(CION_BLADDER_FILELIST)
	./merge_RBEmax_scan_samples $@ $(CION_BLADDER_FILELIST) && \
		rm -f $@.backup $(CION_BLADDER_FILELIST) $(foreach N,$(CION_BLADDER_FILELIST),log_$(N).txt)


$(CION_RECTUM_FILELIST): make_RBEmax_scan_samples
	./make_RBEmax_scan_samples $@ $(N_SAMPLES) 'data/VMATdvh/vmat%d.mat' 25 1 \
		...
		$(RBEMAX_CION_RECTUM_LOW) $(RBEMAX_CION_RECTUM_HIGH) \
		> log_$@.txt 2>&1


RBEmax_scan_cion_rectum_sample_data.mat: merge_RBEmax_scan_samples $(CION_RECTUM_FILELIST)
	./merge_RBEmax_scan_samples $@ $(CION_RECTUM_FILELIST) && \
		rm -f $@.backup $(CION_RECTUM_FILELIST) $(foreach N,$(CION_RECTUM_FILELIST),log_$(N).txt)


$(PROTON_BLADDER_FILELIST): make_RBEmax_scan_samples
	./make_RBEmax_scan_samples $@ $(N_SAMPLES) 'data/VMATdvh/vmat%d.mat' 25 1 \
		...
		$(RBEMAX_PROTON_BLADDER_LOW) $(RBEMAX_PROTON_BLADDER_HIGH) \
		> log_$@.txt 2>&1


RBEmax_scan_proton_bladder_sample_data.mat: merge_RBEmax_scan_samples $(PROTON_BLADDER_FILELIST)
	./merge_RBEmax_scan_samples $@ $(PROTON_BLADDER_FILELIST) && \
		rm -f $@.backup $(PROTON_BLADDER_FILELIST) $(foreach N,$(PROTON_BLADDER_FILELIST),log_$(N).txt)


$(PROTON_RECTUM_FILELIST): make_RBEmax_scan_samples
	./make_RBEmax_scan_samples $@ $(N_SAMPLES) 'data/VMATdvh/vmat%d.mat' 25 1 \
		...
		$(RBEMAX_PROTON_RECTUM_LOW) $(RBEMAX_PROTON_RECTUM_HIGH) \
		> log_$@.txt 2>&1


RBEmax_scan_proton_rectum_sample_data.mat: merge_RBEmax_scan_samples $(PROTON_RECTUM_FILELIST)
	./merge_RBEmax_scan_samples $@ $(PROTON_RECTUM_FILELIST) && \
